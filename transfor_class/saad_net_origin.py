...
class AnchorFreeHead(nn.Module):
    ...
    def __init__(
        ...
        # Per-level learnable scale (FCOS  )
    ...
    def forward(
        ...
        # exp scale ->   ( )
        bbox_pred  = torch.exp(self.scales * self.reg_head(reg_feat))

        # Centerness: LAA anomaly map     
        ...
        if anomaly_map is not None:
            ...
            centerness = centerness + am  #   centerness 

        return cls_logits, bbox_pred, torch.sigmoid(centerness)
    ...
    def forward(
        ...
        if self.use_detection_head:
            ...
            for i, p in enumerate(pyramids):
                ...
                cls_l, bbox_p, ctr = self.det_head(p, am)
                det_outputs.append({
                    'cls_logits': cls_l,
                    'bbox_pred' : bbox_p,
                    'centerness': ctr,
    ...
    def forward(
            ...
            for lvl_idx, out in enumerate(det_outputs):
                ...
                for b in range(B_):
                    ...
                    with torch.cuda.amp.autocast(enabled=False):
                        total_ctr = total_ctr + F.binary_cross_entropy(
        ...
        # -- Weakly supervised fallback ------------------------------------
            ...
            for out in det_outputs:
                ...
                with torch.cuda.amp.autocast(enabled=False):
                    ctr_loss_total = ctr_loss_total + F.binary_cross_entropy(ctr.float(), ctr_target.float())
